<#
.SYNOPSIS
  Use this script to copy an existing Nutanix Flow application rule to a new rule on the same Prism Central.
.DESCRIPTION
  Given a Prism Central, a source and destination rule name, an AppType category and a policy mode (enforce/monitor) the script will copy the source rule to a new rule with the desired name and apply it to the specified AppType category.
.PARAMETER help
...
.PARAMETER sourceRule
  Name of the Flow policy you want to copy.
.PARAMETER targetRule
  Name of the new Flow policy you want to create from the source rule.
.PARAMETER appType
  Value of the AppType category you want the new rule to apply to.
.PARAMETER mode
  Specifies if you want the new rule to be in enforce or monitor mode. If this is not specified, it will use the same mode as the source rule.
.EXAMPLE
.\Copy-FlowRule.ps1 -pc pc1.local -prismCreds myadcreds -sourceRule myFlowRule1 -targetRule myFlowRule2 -appType app2 -mode monitor
Copy myFlowRule1 to myFlowRule2 and apply it to AppType app2 in monitor mode:
.LINK
...
#region parameters
    ...
    (
        ...
        [parameter(mandatory = $true)] [string]$sourceRule,
        [parameter(mandatory = $true)] [string]$targetRule,
        [parameter(mandatory = $true)] [string]$appType,
        [parameter(mandatory = $false)] [ValidateSet("enforce","monitor")] [string]$mode,
...
'@
    ...
    {#we want to create a log transcript
        $myvar_output_log_file = (Get-Date -UFormat "%Y_%m_%d_%H_%M_") + "Copy-FlowRule.log"
...
#region main
    ...
    #region GET rules
        ...
        $source_rule = $rules_response | Where-Object {$_.spec.name -eq $sourceRule}
        Write-Host "$(get-date) [DATA] There are $($source_rule.count) Flow rules which match name $($sourceRule) on Prism Central $($pc)..." -ForegroundColor White

        if (!$source_rule)
        {#we didn't find any matching rules
            Write-Host "$(get-date) [ERROR] There are no Flow rules on $($pc) which match name $($rules)!" -ForegroundColor Red
            exit 1
        }
    #endregion GET rules
    
    ForEach ($rule in $source_rule)
    {#process each rule in source_rule (as multiple rules may have the same name which mostly applies when rule is filtered on app tiers)
        $rule.psobject.members.remove("status") #remove the status section
        $rule.metadata.psobject.members.remove("uuid") #remove unique reference in metadata
        $rule.metadata.psobject.members.remove("owner_reference") #remove owner reference in metadata
        if ($rule.spec.resources.app_rule)
        ...
        {
            Write-Host "$(Get-Date) [WARNING] Rule $($rule.spec.Name) is not an application policy/rule: skipping!" -ForegroundColor Yellow
            continue
        }
        #* apply target rule name
        $rule = ($rule | ConvertTo-Json -Depth 100) -replace "$($sourceRule)","$($targetRule)" | ConvertFrom-Json
        ...
        {#we couldn't create the network policy
            if (($_.Exception.Message -match 'rule already exists') -or ($_.Exception.Message -match 'is already part of rule'))
            {#the isolation rule already exists, let's just warn about this
                Write-Host "$(Get-Date) [WARNING] Could not add rule $($rule.spec.Name) to $($pc) because it already exists or there is already an application policy for AppType $($appType)!" -ForegroundColor Yellow
