...
class AlertsApi(object):
        ...
        try:
            if kwargs.get('async_req'):
                return self.api_client._call_api(
                    '/api/monitoring/v4.0/serviceability/alerts/{extId}', 'GET',
                    path_params,
                    query_params,
                    header_params,
                    body=body_params,
    ...
    def list_alerts(self, _page=None, _limit=None, _filter=None, _orderby=None, _select=None, **kwargs):  # noqa: E501
        ...
        :type _limit:
            :class:`int`
        :param _filter: A URL query parameter that allows clients to filter a collection of resources. The expression specified with $filter is evaluated for each resource in the collection, and only items where the expression evaluates to true are included in the response. Expression specified with the $filter must conform to the [OData V4.01](https://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html) URL conventions. For example, filter '$filter=name eq 'karbon-ntnx-1.0' would filter the result on cluster name 'karbon-ntnx1.0', filter '$filter=startswith(name, 'C')' would filter on cluster name starting with 'C'. The filter can be applied to the following fields: - alertType - classifications - extId - impactTypes - isAcknowledged - isAutoResolved - isResolved - originatingClusterUUID - serviceName - severity - sourceEntity/extId - sourceEntity/name - sourceEntity/type 
        :type _filter:
            :class:`str`
        :param _orderby: A URL query parameter that allows clients to specify the sort criteria for the returned list of objects. Resources can be sorted in ascending order using asc or descending order using desc. If asc or desc are not specified, the resources will be sorted in ascending order by default. For example, '$orderby=templateName desc' would get all templates sorted by templateName in descending order. The orderby can be applied to the following fields: - creationTime - lastUpdatedTime 
        :type _orderby:
            :class:`str`
        :param _select: A URL query parameter that allows clients to request a specific set of properties for each entity or complex type. Expression specified with the $select must conform to the [OData V4.01](https://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html) URL conventions. If a $select expression consists of a single select item that is an asterisk (i.e., *), then all properties on the matching resource will be returned. - acknowledgedByUsername - acknowledgedTime - affectedEntities - alertType - classifications - clusterUUID - creationTime - extId - impactTypes - isAcknowledged - isAutoResolved - isResolved - isUserDefined - lastUpdatedTime - links - message - metricDetails - originatingClusterUUID - parameters - resolvedByUsername - resolvedTime - rootCauseAnalysis - serviceName - severity - severityTrails - sourceEntity/extId - sourceEntity/name - sourceEntity/type - tenantId - title 
        :type _select:
            :class:`str`
        :param kwargs: kwargs for the method.
                       The following can be passed for an asynchronous HTTP call::
        ...
        try:
            if kwargs.get('async_req'):
                return self.api_client._call_api(
                    '/api/monitoring/v4.0/serviceability/alerts', 'GET',
                    path_params,
                    query_params,
                    header_params,
                    body=body_params,
