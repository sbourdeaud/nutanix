""" creates security policies for Flow NextGen using basic NVD templates.
    ...
    Args:
        ...
        action: add or remove policies.
        state: SAVE, MONITOR or ENFORCE.
...
def main(api_server,username,secret,policy_type,scope,vpc_name,qty,action,policy_state,max_workers=5,secure=False):
    '''main function.
        Args:
    ...
    if scope == "VPC_LIST" and action == "add":
            ...
            if category_key not in unique_existing_category_keys:
                ...
                for category_value in category_values:
                    ...
                    if not category_match:
                        ...
                        new_category.description = "Created by add_security_policy.py script"
                        response = entity_api.create_category(new_category)
        #* grabbing categories again as we just added some to the list
        #region GET categories
        api_client_configuration = ntnx_prism_py_client.Configuration()
        api_client_configuration.host = api_server
        api_client_configuration.username = username
    ...
    #* ADD or REMOVE required policies
    ...
    for security_policy in required_policies_list:
        ...
        if not security_policy_match and action == "add":
            ...
            #! this is giving an error which was reported: AttributeError: 'SecurityPolicyState' object has no attribute 'swagger_types'
            #new_policy.state = ntnx_microseg_py_client.SecurityPolicyState(policy_state)
            ...
            new_policy.rules = []
            if policy_type.upper() == "APPLICATION":
                ...
                for app_tier in app_tier_list:
                    ...
                    application_security_policy_rule.spec = security_policy_rule_spec
                    new_policy.rules.append(application_security_policy_rule)
            elif policy_type.upper() == "ISOLATION":
                isolation_security_policy_rule = ntnx_microseg_py_client.NetworkSecurityPolicyRule()
                isolation_security_policy_rule.description = f"Isolation rule for all tiers in application {security_policy}"
                isolation_security_policy_rule.type = "MULTI_ENV_ISOLATION"

                security_policy_rule_spec = ntnx_microseg_py_client.MultiEnvIsolationRuleSpec()
                all_to_all_isolation_group = ntnx_microseg_py_client.AllToAllIsolationGroup()
                all_to_all_isolation_group.isolation_groups = []

                #adding all AppTiers to the isolation group for this AppType
                for app_tier in app_tier_list:
                    isolation_group = ntnx_microseg_py_client.IsolationGroup()
                    isolation_group.group_category_references = [str(next(iter(app_type_uuid)))]
                    app_tier_uuid = {c.ext_id for c in categories_list if (c.key == "AppTier" and c.value == app_tier)}
                    isolation_group.group_category_references.append(str(next(iter(app_tier_uuid))))
                    all_to_all_isolation_group.isolation_groups.append(isolation_group)

                security_policy_rule_spec.spec = all_to_all_isolation_group
                isolation_security_policy_rule.spec = security_policy_rule_spec
                new_policy.rules.append(isolation_security_policy_rule)
            #* create policy
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--action","-a",
        choices=["add","remove"],
        help="Specifies if you want to add or remove policies.",
        default="add"
    )
    parser.add_argument("--state",
        choices=["SAVE","MONITOR","ENFORCE"],
        help="Specifies the state of the policies to be created.",
        default="SAVE"
    )
    ...
    main(api_server=args.prism,username=args.username,secret=pwd,
         action=args.action,
         policy_type=args.type,scope=args.scope,vpc_name=args.vpc_name,qty=args.qty,policy_state=args.state,
         max_workers=args.threads,secure=args.secure)
