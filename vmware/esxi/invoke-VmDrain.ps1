<#
.SYNOPSIS
  You can use this script to move all vms (except the cvm) off an ESXi host to 
  a pre-defined group of hosts.
.DESCRIPTION
  Using pre-defined groups of hosts (defined as constants in the script), move 
  all vms (except the cvm) using vmotion to the host in the same group as the 
  specified host which has the least running VMs.
.PARAMETER help
...
.PARAMETER vcenter
  VMware vCenter server hostname. Default is localhost. You can specify several 
  hostnames by separating entries with commas.
.PARAMETER vmhost
...
Move all VMs (except the cvm) off host myhost01 in vcenter myvcenter.local. VMs 
will be moved to hosts in the same group as myhost01.
.LINK
...
#check PoSH version
if ($PSVersionTable.PSVersion.Major -lt 5) {throw "$(get-date) [ERROR] Please `
upgrade to Powershell v5 or above (https://www.microsoft.com/en-us/download/ `
details.aspx?id=50395)"}

#check if we have all the required PoSH modules
Write-Host "$(get-date) [INFO] Checking for required Powershell modules..." `
...
#region Load/Install VMware.PowerCLI
    ...
    {#module VMware.PowerCLI is not loaded
        ...
        {#load module VMware.PowerCLI
            Write-Host "$(get-date) [INFO] Loading VMware.PowerCLI module..." `
            -ForegroundColor Green
            Import-Module VMware.PowerCLI -ErrorAction Stop
            Write-Host "$(get-date) [SUCCESS] Loaded VMware.PowerCLI module" `
            -ForegroundColor Cyan
        }
        catch 
        {#couldn't load module VMware.PowerCLI
            Write-Host "$(get-date) [WARNING] Could not load VMware.PowerCLI `
            module!" -ForegroundColor Yellow
            try 
            {#install module VMware.PowerCLI
                Write-Host "$(get-date) [INFO] Installing VMware.PowerCLI `
                module..." -ForegroundColor Green
                Install-Module -Name VMware.PowerCLI -Scope CurrentUser `
                -ErrorAction Stop
                Write-Host "$(get-date) [SUCCESS] Installed VMware.PowerCLI `
                module" -ForegroundColor Cyan
                try 
                {#loading module VMware.PowerCLI
                    Write-Host "$(get-date) [INFO] Loading VMware.PowerCLI `
                    module..." -ForegroundColor Green
                    Import-Module VMware.VimAutomation.Core -ErrorAction Stop
                    Write-Host "$(get-date) [SUCCESS] Loaded VMware.PowerCLI `
                    module" -ForegroundColor Cyan
                }
                catch 
                {#couldn't load module VMware.PowerCLI
                    throw "$(get-date) [ERROR] Could not load the `
                    VMware.PowerCLI module : $($_.Exception.Message)"
                }
            }
            catch 
            {#couldn't install module VMware.PowerCLI
                throw "$(get-date) [ERROR] Could not install the `
                VMware.PowerCLI module. Install it manually from `
                https://www.powershellgallery.com/items?q=powercli&x=0&y=0 : `
                $($_.Exception.Message)"
            }
        }
    }
    ...
    {#check PowerCLI version
        ...
        {#couldn't update module VMware.PowerCLI
            throw "$(get-date) [ERROR] Could not update the VMware.PowerCLI `
            module : $($_.Exception.Message)"
        }
    }
#endregion

if ((Get-PowerCLIConfiguration | where-object `
{$_.Scope -eq "User"}).InvalidCertificateAction -ne "Ignore") {
...
#region cleanup
    ...
    Write-Host "$(get-date) [SUM] total processing time: `
    $($myvarElapsedTime.Elapsed.ToString())" -ForegroundColor Magenta
