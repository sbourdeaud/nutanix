...
# description:  Given a network, vm name and domain, reserve an ip
#               in Infoblox.
# input:        network (exp: 192.168.0.0/24)
# output:       ip_ref (infoblox reference to reserved ip object)
#               vm_ip (ipv4 address)
"""
# endregion
...
api_server_endpoint = "/wapi/v2.7/fixedaddress"
url = "https://{}:{}{}".format(
    api_server,
    api_server_port,
    api_server_endpoint
...
method = "POST"
payload = {
    "ipv4addr": "func:nextavailableip:{}".format(network),
    "name": "{0}.{1}".format(vm_name,domain),
...
if resp.ok:
    ...
    ip_ref = resp.content
    vm_ip = (((ip_ref.split(":"))[1]).split("/"))[0]
    print("ip_ref= {}".format(ip_ref))
    print("vm_ip= {}".format(vm_ip))
else:
    exit(1)
