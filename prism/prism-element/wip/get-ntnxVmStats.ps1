...
#TODO: finish adding bits for vm processing
...
#region retrieving stats for all metrics
    if ($vm) {
        ...
        if ($vm -eq "all") {
            #TODO: compile list of all vm uuids
            ForEach ($entity in $vms.entities) {

            }
        } else {
            #TODO: get uuid of target vm
        }
    }
    
    ForEach ($metric in $myvar_metrics_list) {
        if ($vm) {
            #TODO: foreach vm uuid retrieve stat
        } else {
...
#region exporting results to csv
    ForEach ($metric in $myvar_metrics_results.keys) {
        #TODO: add specific processing for vms
...
#region displaying graphs
    if ($graph) {
        ...
        #TODO: add specific procesing for vms
